...
class AI(Player): 
    def __init__(self, name, sign, board):
        Player.__init__(self, name, sign) #calls person to initialize
        self.board = board #initializes board as well
        
    def choose (self):
        board = self.board
        boardEmpty = [] #sets list for indexes with whitespaces
        for m in range (0,9):
            if board.isempty(m) == True:
                boardEmpty.append(m)
        index = choice(boardEmpty) #gets random value from possibilities
        board = board.set(index, self.sign) #sets and updates the board
        return board #returns the board

class SmartAI (AI):
    def choose (self):
        if self.sign == "X": #the point of this if else is to detemrine the opposing sign
            opposeSign = "O"
        else:
            opposeSign = "X"
        board = self.board
        possible = []
        oppose = []
        for n in range (0,9): #detmerines best possible move
            if board.isempty(n) == True:
                possible.append(n)
            else:
                oppose.append(n)
        option = choice (possible)
        board = board.set(option, self.sign) #sets and updated the board
        return board #returns the board
